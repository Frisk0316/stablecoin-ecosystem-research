...
| --- | --- | --- | --- | --- | --- |
| **適格發行者**<br/>Eligible issuer | Permitted payment stablecoin issuer only; 3-年 gate + Section 18 foreign-issuer screen<br/>(`CLAIM_082`, `CLAIM_083`, `CLAIM_126`) | Authorised ART issuer; significant-ART 由 EBA 監管<br/>(`CLAIM_094`) | Credit institution 或 e-money institution; significant-EMT 可能由 EBA 或 home state 監管<br/>(`CLAIM_097`, `CLAIM_127`, `CLAIM_129`) | Recognised systemic stablecoin issuer; Bank / FCA 共同監管，cross-border use 仍 subject to home authority<br/>(`CLAIM_088`, `CLAIM_090`, `CLAIM_130`) | DFS-regulated virtual currency entity issuing under DFS supervision<br/>(`CLAIM_036`) |
...
| **贖回權**<br/>Redemption right | 發行人必須對 fixed monetary value 提供 convert / redeem / repurchase 義務 (Sec. 2 definition); rehypothecation 原則禁止<br/>(`CLAIM_084`) | 對發行人之 permanent redemption right; redemption in funds 或 referenced assets; 原則無 fee（Art. 46 例外）<br/>(`CLAIM_070`, `CLAIM_095`) | At-any-time, par-value redemption from the issuer<br/>(`CLAIM_042`, `CLAIM_122`, `CLAIM_124`) | Robust legal claim + always-at-par fiat redemption；backing / recovery 設計支撐贖回<br/>(`CLAIM_089`, `CLAIM_131`) | Lawful-holder at-par redemption in a timely fashion，預設 T+2 after compliant order<br/>(`CLAIM_038`) |
...
- 提醒：「Lawful holder」與「Customer」不可混為一談；foreign-issuer equivalence 也不可從比較表直接推論，需回到 Section 18 screen。
